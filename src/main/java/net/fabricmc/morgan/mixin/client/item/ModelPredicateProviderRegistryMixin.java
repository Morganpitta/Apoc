package net.fabricmc.morgan.mixin.client.item;

import net.fabricmc.morgan.item.MorganItems;
import net.fabricmc.morgan.item.TrackingCompassItem;
...
public abstract class ModelPredicateProviderRegistryMixin {
    ...
    static {
        ModelPredicateProviderRegistry.register(MorganItems.TRACKING_COMPASS, new

                Identifier("angle"), new UnclampedModelPredicateProvider() {
                    ...
                    @Override
                    public float unclampedCall(ItemStack itemStack, @Nullable ClientWorld clientWorld, @Nullable LivingEntity
                            livingEntity, int i) {
                        double g;
                        Entity entity;
                        ...
                        BlockPos blockPos = TrackingCompassItem.hasLodestone(itemStack) ? this.getLodestonePos(clientWorld, itemStack.getOrCreateNbt()) : this.getSpawnPos(clientWorld);
                    ...
                    private BlockPos getLodestonePos(World world, NbtCompound nbt) {
                        ...
                        if (bl && bl2 && (optional = TrackingCompassItem.getLodestoneDimension(nbt)).isPresent() && world.getRegistryKey() == optional.get()) {
